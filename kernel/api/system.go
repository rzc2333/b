...
func setDownloadInstallPkg(c *gin.Context) {
	...
	// --- 注入：强制拦截 ---
	model.Conf.System.DownloadInstallPkg = false // 不管前端点没点，我这里永远是 false
	model.Conf.Save()                             // 确保这个“关闭”被存入硬盘
	return                                        // 直接走人，后面什么解析逻辑都不跑了
	// --------------------
...
func setNetworkProxy(c *gin.Context) {
	...
	// 注入：主权隔离，禁止修改网络代理配置
	ret.Code = -1
	ret.Msg = "Network Proxy configuration is locked by Protocol."
	return 
